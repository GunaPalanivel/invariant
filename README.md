...
### Scorecard (measured, same candidate)

Same generated bytes, SHA-256 `0154894c447e76d3ff80105524bf5b9d3b4bf5732fff97ad6036eaca4e407a19`. Source: [`results/execution-manifest.json`](results/execution-manifest.json), [`results/evaluate.json`](results/evaluate.json), [`results/comparison.json`](results/comparison.json). Local suite: **64 tests passed**.

| Rubric | Weight | Measured |
| --- | --- | --- |
| Technical execution | 30% | 3 live apps (Slack + Linear + GitHub PR 1). Isolated subprocess matrix, env allowlist, CI binds the observed blob hash. AUT never sees World/COMMITTED labels. `pack.valid` needs matrix **and** an independent checker. |
| Reliability & evaluation | 25% | **4 overlays × 4 tests, one hash.** Original: 2 required failures (lost-ack + empty-page). Incomplete: 1 required failure (empty-page). Correct: **4/4 pass**, exit 0. Content-dedup mutant: new-op **fails**. Checker: **3 findings + 4 passes**, 0 `invalid_test`, 0 infra failures. Holdout **4/4**. Hostile canary: `followed_admin_channel=false`, `skip_verification=false`. Unknown is not painted complete. |
| Usefulness | 20% | Weak control **false-accepts** the original duplicate (`observer` count 2). Invariant **rejects** that duplicate and the incomplete empty-page retry, then publishes **one** PR, **one** Slack finding, **one** Linear comment (journal GET-reuse, no second write). Local pack wall **1.13s**. |
| Originality | 15% | Incident thread → grounded contract with source spans → verified regression PR. Identity is `operation_id`, not message text. Groq scenario spec compiled deterministically (`scenario-compiler:model:groq`). Interpret usage: **1725 in / 811 out**. |
| Demo clarity | 10% | [2-minute video](https://drive.google.com/file/d/1tb08gPVp2HEMIAyEtVWJgQBj0cxEJO9x/view?usp=sharing), [PR 1](https://github.com/GunaPalanivel/invariant-validation/pull/1), and the Slack/Linear screenshots below. No workspace login required to score the run. |
