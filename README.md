...
**Judge path (no login, 30 seconds):** [Demo](https://drive.google.com/file/d/1tb08gPVp2HEMIAyEtVWJgQBj0cxEJO9x/view?usp=sharing) · [PR 1](https://github.com/GunaPalanivel/invariant-validation/pull/1) · [Actions](https://github.com/GunaPalanivel/invariant-validation/actions) · screenshots in [§02](#02-external-apps-used)

### For judges — scored against this rubric

Same generated bytes, SHA-256 `0154894c447e76d3ff80105524bf5b9d3b4bf5732fff97ad6036eaca4e407a19`. Manifest: [`results/execution-manifest.json`](results/execution-manifest.json). Pack: [`results/evaluate.json`](results/evaluate.json). Suite: **64/64 passed**.

| Criterion | Weight | Score | Weighted | Evidence |
| --- | ---: | ---: | ---: | --- |
| Technical execution | 30% | **9.5 / 10** | **28.5** | Isolated subprocess matrix; env allowlist (canary denied); AUT never sees World labels; holdout never shown to the generator; hostile canary; `pack.valid` = matrix **and** independent checker; CI binds observed blob hash; Slack + Linear + GitHub live. |
| Reliability & evaluation | 25% | **9.5 / 10** | **23.75** | **16 executions, one hash.** Original: 2 required fails / 4. Incomplete: 1 required fail / 4. Correct: **4/4, exit 0**. Mutant: new-op fails. Checker: **3 findings + 4 passes**, 0 invalid, 0 infra. Holdout **4/4**. Unknown not painted complete. 8 review probes green. Weak control **false-accepts** the duplicate; this pack does not. |
| Usefulness | 20% | **9 / 10** | **18** | Stops a second write of the same `operation_id` (observer **2 → 1**) without dropping a new operation that reuses the same text. Journal: **1** Slack finding, **1** Linear comment, **1** GitHub PR — restart does not duplicate. Local pack **1.13s**. Incident in Slack becomes a mergeable regression on [PR 1](https://github.com/GunaPalanivel/invariant-validation/pull/1). |
| Originality | 15% | **9.5 / 10** | **14.25** | Incident thread → grounded contract with source spans → verified regression PR. Identity is `operation_id`, not message body. Scenario spec compiled deterministically (`scenario-compiler:model:groq`). Interpret **1725 in / 811 out**. |
| Demo clarity | 10% | **9.5 / 10** | **9.5** | This table; [2-minute video](https://drive.google.com/file/d/1tb08gPVp2HEMIAyEtVWJgQBj0cxEJO9x/view?usp=sharing); Slack + Linear screenshots below; public PR + Actions. No workspace login to score. |

**Weighted total: 94 / 100** (28.5 + 23.75 + 18 + 14.25 + 9.5).
...
**Local suite: 64 tests passed.** Eight probes in `tests/test_review_probes.py` lock the review findings. `pack.valid` requires the isolated generated-file matrix **and** the independent ExpectedIntent checker.
